...
#[path = "../../../webpush-crypto/tests/oracle/mod.rs"]
mod oracle;
pub use oracle::*;

use std::{fmt::Write as _, future::Future, net::SocketAddr, pin::Pin, sync::Arc, time::Duration};
...
/// How to start a [`TestServer`]. The defaults give a single node with the
/// memory store (or Bigtable, with `PUSH_TEST_STORE=bigtable`).
#[derive(Default)]
pub struct Options {
    /// `role = ...`, when not `all`.
    pub role: Option<&'static str>,
    /// Public origin, when it is not this server's own address: connection
    /// nodes issue endpoints on the endpoint node's origin.
    pub origin: Option<String>,
    /// Extra TOML tables appended to the configuration.
    pub toml: String,
    /// Bridges to install.
    pub bridges: Vec<webpush_bridge::SharedBridge>,
    /// Join a cluster with this token: adds an internal listener and
    /// `[cluster]` with this node's internal URL.
    pub cluster_token: Option<String>,
    /// A store shared with other servers, instead of a fresh one.
    pub store: Option<webpush_store::MemoryStore>,
    /// Add an internal listener (implied by `cluster_token`).
    pub internal: bool,
}

/// A push service under test. With `PUSH_TEST_STORE=bigtable` (and the
/// `bigtable` feature) the store is a Bigtable emulator started for this
/// server alone. Dropping it starts a graceful shutdown.
pub struct TestServer {
    /// `https://localhost:{port}`: the base of every URI the server issues.
    pub origin: String,
    /// Address to connect to; `localhost` in `origin` resolves here.
    pub addr: SocketAddr,
    /// The internal listener, when one was requested.
    ...
    _emulator: Option<bigtable_emulator::Emulator>,
    /// Starts the graceful shutdown.
    stop: Option<tokio::sync::oneshot::Sender<()>>,
    /// The serving task.
    server: Option<JoinHandle<Result<(), webpush_server::BoxError>>>,
}

#[cfg(not(feature = "bigtable"))]
mod bigtable_emulator {
    /// Stands in for the emulator handle in builds without Bigtable.
    pub struct Emulator;
}
#[cfg(feature = "bigtable")]
use bigtable as bigtable_emulator;

/// Write `contents` to a fresh file in the temp directory.
fn temp_file(name: &str, contents: &[u8]) -> std::path::PathBuf {
    let path = std::env::temp_dir().join(format!("webpush-test-{}-{name}", b64(&random::<9>())));
    std::fs::write(&path, contents).unwrap();
    path
}

impl TestServer {
    /// Start a single node on a free port with a fresh self-signed
    /// certificate for `localhost`.
    pub async fn start() -> TestServer {
        Self::start_with(Options::default()).await
    }

    /// Start a server as `opts` describes.
    #[allow(clippy::unnecessary_debug_formatting)]
    pub async fn start_with(opts: Options) -> TestServer {
        let cert = rcgen::generate_simple_self_signed(vec!["localhost".to_owned()]).unwrap();
        let cert_file = temp_file("cert.pem", cert.cert.pem().as_bytes());
        let key_file = temp_file("key.pem", cert.signing_key.serialize_pem().as_bytes());
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let own_origin = format!("https://localhost:{}", addr.port());
        let origin = opts.origin.clone().unwrap_or_else(|| own_origin.clone());

        let mut toml = String::new();
        if let Some(role) = opts.role {
            writeln!(toml, "role = \"{role}\"").unwrap();
        }
        write!(
            toml,
            // Debug formatting quotes and escapes the paths, as TOML needs.
            "origin = \"{origin}\"\n\
             [public.tls]\ncert_file = {cert_file:?}\nkey_file = {key_file:?}\n"
        )
        .unwrap();
        // Test defaults, unless the test sets the table itself.
        if !opts.toml.contains("[push]") {
            toml.push_str("[push]\nreaper_interval = \"100ms\"\n");
        }
        if !opts.toml.contains("[shutdown]") {
            toml.push_str("[shutdown]\ndrain_delay = \"0s\"\ntimeout = \"5s\"\n");
        }
        let mut internal = None;
        if opts.internal || opts.cluster_token.is_some() {
            let l = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
            let a = l.local_addr().unwrap();
            writeln!(toml, "[internal]\nlisten = \"{a}\"").unwrap();
            if let Some(token) = &opts.cluster_token {
                write!(
                    toml,
                    "[cluster]\nnode_url = \"http://{a}\"\ntoken = \"{token}\"\n"
                )
        ...
        toml.push_str(&opts.toml);
        let cfg = webpush_server::config::Config::from_toml(&toml)
            .unwrap_or_else(|e| panic!("test config: {e}\n{toml}"));
        let internal_addr = internal.as_ref().map(|l| l.local_addr().unwrap());
        ...
        macro_rules! launch {
            ($store:expr) => {{
                let mut server =
                    webpush_server::Server::new(cfg, $store).listeners(listener, internal);
                for b in opts.bridges {
                    server = server.bridge(b);
                }
                tokio::spawn(server.run(signal))
            }};
        }
        let backend = std::env::var("PUSH_TEST_STORE").unwrap_or_default();
        let (emulator, server) = match (backend.as_str(), opts.store) {
            (_, Some(shared)) => (None, launch!(shared)),
            ("" | "memory", None) => (None, launch!(webpush_store::MemoryStore::new())),
            #[cfg(feature = "bigtable")]
            ("bigtable", None) => {
                let (emulator, store) = bigtable::start(60 * 24 * 3600).await;
                (Some(emulator), launch!(store))
            }
            (other, None) => panic!("PUSH_TEST_STORE={other} is not supported by this build"),
        };
        TestServer {
            origin: own_origin,
            addr,
    ...
    pub async fn shutdown(mut self) {
        ...
        if let Some(server) = self.server.take() {
            timeout(Duration::from_secs(10), server)
                .await
                .expect("graceful shutdown within 10 s")
                .unwrap()
                .unwrap();
        }
    }
...
/// Bigtable emulator setup, shared with the store crate's contract tests.
#[cfg(feature = "bigtable")]
#[path = "../../../webpush-store/tests/emulator/mod.rs"]
pub mod bigtable;
...
impl Ua {
    ...
    /// The code of the server's close frame, skipping other messages.
    /// `None` if the connection ends without one or stays open for 5 s.
    pub async fn close_code(&mut self) -> Option<u16> {
        use futures_util::StreamExt;
        timeout(WAIT, async {
            loop {
                match self.ws.next().await? {
                    Ok(Frame::Close(frame)) => return frame.map(|f| u16::from(f.code)),
                    Err(_) => return None,
                    Ok(_) => {}
                }
            }
    ...
    /// Whether a WebSocket ping arrives within `ms` milliseconds, skipping
    /// other frames.
    pub async fn pinged_within(&mut self, ms: u64) -> bool {
        use futures_util::StreamExt;
        timeout(Duration::from_millis(ms), async {
            loop {
                match self.ws.next().await {
                    Some(Ok(Frame::Ping(_))) => return true,
                    Some(Ok(_)) => {}
                    _ => return false,
                }
            }
